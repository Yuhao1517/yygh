...
public class ScheduleController {
    ...
    @ApiOperation(value = "查询排班规则数据2")
    @GetMapping("getScheduleRule2/{page}/{limit}/{hoscode}/{depcode}")
    public Result getScheduleRule2(@PathVariable("page") Integer page,
                                  @PathVariable("limit") Integer limit,
                                  @PathVariable("hoscode") String hoscode,
                                  @PathVariable("depcode") String depcode){
        Map<String,Object> map = scheduleService.getRuleSchedule2(page,limit,hoscode,depcode);
        return Result.ok(map);
    }
}
