...
import com.yh.yygh.hosp.pojo.*;
import com.yh.yygh.hosp.pojo.vo.BookingScheduleRuleVo;
import com.yh.yygh.hosp.repository.ScheduleRepository;
...
//            schedule.setWorkDate(dateAdd(schedule.getWorkDate()));
    ...
    @Override
    public Page<Schedule> findPageSchedule(int page, int limit, String hoscode, String depcode) {
    ...
    public Map<String, Object> getRuleSchedule2(Integer page, Integer limit, String hoscode, String depcode) {
        // 根据医院编号 和 科室编号 查询
        List<Schedule> scheduleList = scheduleRepository.getScheduleByHoscodeAndDepcode(hoscode, depcode);

        //按工作日期进行分组
        Map<Date, List<Schedule>> scheduleMap = scheduleList.stream()
        ...
        for (Map.Entry<Date, List<Schedule>> entry : scheduleMap.entrySet()) {
            BookingScheduleRuleVo bkvo = new BookingScheduleRuleVo();
            //日期
            Date workDate = entry.getKey();
            //日期对应的排班数据
            List<Schedule> schedules = entry.getValue();
            bkvo.setWorkDate(workDate);
            bkvo.setDayOfWeek(getDayOfWeek(new DateTime(workDate)));
            //号源数量
            int docCount = schedules.size();
            bkvo.setDocCount(docCount);
            //利用stream流获取排班list中字段总和
            int reservedNumber = schedules.stream().mapToInt(Schedule::getReservedNumber).sum();
            bkvo.setReservedNumber(reservedNumber);
            int availableNumber = schedules.stream().mapToInt(Schedule::getAvailableNumber).sum();
            bkvo.setAvailableNumber(availableNumber);

            bookingScheduleRuleVoList.add(bkvo);
        }
        //排序 分页
        List<BookingScheduleRuleVo> collect = bookingScheduleRuleVoList.stream()
                .sorted(Comparator.comparing(BookingScheduleRuleVo::getWorkDate).reversed())
                .skip((page - 1) * limit).limit(limit).collect(Collectors.toList());
        //设置最终数据，进行返回
        Map<String, Object> result = new HashMap<>();

        result.put("bookingScheduleRuleList", collect);
        result.put("total", total);
        //获取医院名称
    ...
    private IPage getListDate(Integer page, Integer limit, BookingRule bookingRule) {
        ...
        DateTime releaseTimeOfDateTime = getDateTime(new Date(), releaseTime);
        //获取预约周期
        Integer cycle = bookingRule.getCycle();
        //如果当天放号时间已经过去了，预约周期从后一天开始计算，周期加1
        if(releaseTimeOfDateTime.isBeforeNow()){
            cycle += 1;
        }
